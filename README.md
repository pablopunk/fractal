# 🌀 Fractal

> It's my agent coordinator, not yours. Go away.


<p align="center">
  <img src="https://github.com/user-attachments/assets/7ac50c47-4779-4b31-b7d7-0788682123ca" width="250" />
  <br/>
  <span><i>The app with the ugliest icon you'll ever see</i></span>
  <br/>
  <br/>
  <span><img src="https://cdn-icons-png.flaticon.com/512/9205/9205302.png" width="12" /> <a href="https://github.com/pablopunk/fractal/releases">Get the latest release</a></span>
</p>

## Features

- 💅 **Boring UI**.
- 💻 **Supports ALL CLI agents**. Claude code, pi, codex, you name it. No hacks, just tmux.
- 🗂️ **Turn your ideas into prompts**. Write down all your thoughts, tackle later.
- 🪾 **Worktrees**. Only when you need them.
- 📊 **Kanban style**. Drag and drop prompts like if they were tasks.
- 🔍 **Review column**. PR tracking with CI status, review comments, merge conflicts. Auto-detects PRs and archives when merged.
- 🐙 **GitHub & Linear issues**. Auto-fetched into columns, draggable onto run lanes.
- 🤖 **Fractal Agent**. Built-in AI chat sidebar — bring your own keys, streaming tool calls.
- 🖥️ **Embedded terminal tabs**. xterm.js, per-project, reorderable, multiple themes.
- 🌐 **Remote access**. Tailscale + QR code. Full board + terminals from any device.

<img width="1200" height="745" alt="CleanShot 2026-05-14 at 12 02 49" src="https://github.com/user-attachments/assets/01f498d1-f835-4488-9857-5f0cf1780fcb" />
<img width="1200" height="742" alt="CleanShot 2026-05-14 at 12 03 38" src="https://github.com/user-attachments/assets/31d1226a-ff8f-49d1-b399-ebbd5d0fa56a" />
<img width="1670" height="1178" alt="CleanShot 2026-06-07 at 14 02 21" src="https://github.com/user-attachments/assets/f3d84a1f-5dfa-4276-b686-8b8bd0e552a3" />
<img width="1978" height="1213" alt="CleanShot 2026-06-12 at 14 08 47" src="https://github.com/user-attachments/assets/aaae0f7e-baf1-4ad4-a16b-8138bd429320" />
