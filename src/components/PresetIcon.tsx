...
export default function PresetIcon({ preset, size = 14 }: { preset: Pick<AgentPreset, "kind" | "binary" | "name">; size?: number }) {
  ...
  if (icon.type === "letter") {
    ...
    return <span className="preset-icon preset-icon-letter" style={letterStyle} aria-hidden>{icon.letter}</span>;
  }
  return (
